...
export async function importGrades(gradesToImport: ImportTranscript[]): Promise<Transcript[]> {
  ...
  await Promise.all(gradesToImport.map(async importTranscript => {
    console.log("Creating a student")
    const { studentID } = await client.addStudent(importTranscript.studentName);
    console.log("Adding grades")
    await Promise.all(importTranscript.grades.map(grade => client.addGrade(studentID, grade.course, grade.grade)))
